...
describe('DispatchModal', () => {
  ...
  it('dispatches t.task.notionUrl (not t.task.id) as taskUrl when notionUrl is present', () => {
  ...
  it('leaves taskUrl undefined when notionUrl is absent (YAML-only fallback synthesized server-side)', () => {
    const task = makeTask('yaml-task-id', 'YAML Task', {
      task: {
        id: 'yaml-task-id',
        title: 'YAML Task',
    ...
    >;
    expect(dispatchMsg).toBeDefined();
    expect(dispatchMsg.tasks[0].taskUrl).toBeUndefined();
  });
