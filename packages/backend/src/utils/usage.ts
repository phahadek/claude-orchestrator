...
const MODEL_PRICING: { match: string; inputPerMillion: number; outputPerMillion: number }[] = [
  { match: 'opus',   inputPerMillion: 15,   outputPerMillion: 75 },
  { match: 'sonnet', inputPerMillion: 3,    outputPerMillion: 15 },
  { match: 'haiku',  inputPerMillion: 0.80, outputPerMillion: 4  },
];

/** Default pricing when model is unknown — falls back to Sonnet. */
const DEFAULT_PRICING = MODEL_PRICING[1];

export function calculateCost(inputTokens: number, outputTokens: number, model?: string | null): number {
  const pricing = MODEL_PRICING.find((p) => model?.toLowerCase().includes(p.match)) ?? DEFAULT_PRICING;
