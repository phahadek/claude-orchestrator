 ...
 * DockerSessionRunner, both of which pass `--session-id` on their spawned
 * command line). Deliberately independent of SessionManager's in-memory
...
export function isSessionProcessAlive(sessionId: string): boolean {
  try {
    ...
    return out.includes(`--session-id ${sessionId}`);
  } catch (err) {
    logger.error(
