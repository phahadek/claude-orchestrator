  ...
  async resumeOrphanSessions(): Promise<void> {
    ...
    if (stuckRows.length > 0) {
      ...
      for (const row of stuckRows) {
        markSessionDone(row.session_id, row.last_ts, row.pr_url ?? null, 'boot_orphan_result_event');
        let taskBackend;
        try {
    ...
    if (mergedPrRows.length > 0) {
      ...
      for (const row of mergedPrRows) {
        markSessionDone(row.session_id, row.last_ts, row.pr_url ?? null, 'boot_merged_or_closed_pr');
        let taskBackend;
        try {
          taskBackend = row.project_id ? getTaskBackend(row.project_id) : null;
        } catch {
