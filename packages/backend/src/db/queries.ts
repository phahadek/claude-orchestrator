...
// ─── planning_checkout_locks ────────────────────────────────────────────────

export interface PlanningCheckoutLockRow {
  session_id: string;
  project_dir: string;
  scratch_dir: string;
  created_at: number;
...
/** Registers a planning session as holding the read-only-checkout lockdown for its project. */
export function insertPlanningCheckoutLock(
  sessionId: string,
  projectDir: string,
  scratchDir: string,
): void {
  db.prepare<{
    ...
    `INSERT OR REPLACE INTO planning_checkout_locks
       (session_id, project_dir, scratch_dir, created_at)
     VALUES (@session_id, @project_dir, @scratch_dir, @created_at)`,
  ).run({
    session_id: sessionId,
...
/** Removes a planning session's checkout-lockdown registration. No-op if absent. */
export function deletePlanningCheckoutLock(sessionId: string): void {
  db.prepare<{ session_id: string }>(
    `DELETE FROM planning_checkout_locks WHERE session_id = @session_id`,
  ).run({ session_id: sessionId });
}

export function getPlanningCheckoutLock(
  sessionId: string,
...
/** All lock rows across all projects — boot reconciliation's working set. */
export function getAllPlanningCheckoutLocks(): PlanningCheckoutLockRow[] {
  return db
    .prepare(`SELECT * FROM planning_checkout_locks`)
    .all() as PlanningCheckoutLockRow[];
}
