...
export function resolveTestScratchDataDir(pid: number, backendSrcDir: string = __dirname): string {
  return path.join(backendSrcDir, '..', '.test-scratch-datadir-DO-NOT-COMMIT', `pid-${pid}`);
}
