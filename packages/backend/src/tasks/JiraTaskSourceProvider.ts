...
export class JiraTaskSourceProvider implements TaskBackend {
  ...
  async updateStatus(taskId: string, status: string): Promise<void> {
    ...
    if (!targetJiraStatus) {
      // Orchestrator-only statuses (e.g. 🚫 Blocked, ⏭️ Deferred) have no Jira equivalent — skip.
      logger.warn(
    ...
    if (!transition) {
      ...
      logger.warn(
        `[JiraTaskSourceProvider] no direct transition to "${targetJiraStatus}" for issue ${externalId} (currently "${issue.fields.status.name}") — skipping`,
      );
      return;
    }
