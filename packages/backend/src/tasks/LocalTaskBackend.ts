...
function isMilestoneSchema(parsed: AnyTasksFile | null): parsed is MilestoneTasksFile {
...
export class LocalTaskBackend implements TaskBackend {
  ...
  private readFile(): MilestoneTasksFile {
    if (!fs.existsSync(this.filePath)) {
      throw new Error(`[LocalTaskBackend] tasks.yaml not found at ${this.filePath}`);
    }
    ...
    if (isFlatSchema(parsed)) {
      ...
      console.log(`[LocalTaskBackend] Migrated ${this.filePath} to milestone schema.`);
      return migrated;
    }
  ...
  private migrateFlatToMilestones(flat: FlatTasksFile): MilestoneTasksFile {
    const id = flat.board_id && flat.board_id !== 'default' ? flat.board_id : 'm1';
    const name = flat.board_id && flat.board_id !== 'default' ? flat.board_id : 'Default';
    return {
  ...
  private findTaskById(file: MilestoneTasksFile, taskId: string): { milestone: LocalMilestone; task: LocalTask } | undefined {
  ...
  async fetchReadyTasks(milestoneId: string, _skipCache?: boolean): Promise<ResolvedTask[]> {
