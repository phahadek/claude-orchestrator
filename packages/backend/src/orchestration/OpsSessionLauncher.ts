...
/**
 * Human-triggered per-task ops session launcher behind the Ops(N) button —
 * the individual-session replacement for the old combined ops run. Each
 * selected task launches its own session via SessionManager.start(), the
 * same code path as a manual UI launch. A task whose Depends On isn't all
 * ✅ Done is deferred and retried on the next poll, mirroring AutoLauncher's
 * Depends-On gate — but only for tasks a human already selected. Nothing
 * here is auto-dispatched onto tasks the operator didn't pick.
 */
